...
public class MenuOver extends JMenuBar {
	...
	private void configure() {
		jmFile = createMenu("Menu", 'A');
		
		jmEdit = createMenu("Bearbeiten", 'E');
		
		jmHelp = createMenu("Hilfe", 'H');
		
		jmiExit = createMenuItem(jmFile, "Beenden", 'S', "close_view",
				KeyStroke.getKeyStroke(KeyEvent.VK_F4, InputEvent.ALT_MASK));
		
		
		jmiAdd = createMenuItem(jmEdit, "Hinzufuegen", 'A', "add_obj",
				KeyStroke.getKeyStroke(KeyEvent.VK_INSERT, 0));
		
		
		jmiEdit = createMenuItem(jmEdit, "Bearbeiten", 'E', "edit",
				KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, InputEvent.ALT_MASK));
		
		jmiRemove = createMenuItem(jmEdit, "Loeschen", 'R', "remove",
				KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.ALT_MASK));
		
		
		jmiAbout = createMenuItem(jmHelp, "About Info", 'S', "about",
	...
	private void registerListeners(){
		...
		jmiAbout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				System.out.println("About info test");
			}
		});
	}
